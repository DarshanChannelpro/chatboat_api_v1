    ...
    <x-authentication-card>
        ...
        <form method="POST" action="{{ route('login') }}" id="loginform">
            ...
            <div class="block mt-4 flex justify-between">
                ...
                {{-- <label for="remember_me" class="flex items-center">
                    <a href="{{ route('newcompany.register') }}" class="ml-2 text-sm text-gray-600">Register New Client</a>
                </label> --}}
            </div>
        ...
        <x-slot name="links">
            @if (Route::has('register'))
                <a style=" text-center" class="text-center items-center justify-center   text-sm text-gray-600 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" href="{{ route('register') }}">
                    {{ __('If you don\'t have account you can create new one') }} <span  class="underline">{{__('Signup')}}</span>.
                </a>
            @endif
        </x-slot>
